...
* Flexible com.ve.indicadores.error messaging system
...
## Error messages
The validator function holds a messages object called "message", which itself holds all the com.ve.indicadores.error messages being shown to the user for all sort of validation errors.
...
This object can be extended easily. The idea is to extend it with new keys which represent the name of the field you want the message to be linked to, and that custom message appear as the `general com.ve.indicadores.error` one. Default messages can be over-ride.
Example: for a given type ‘date’ field, lets set a custom (general com.ve.indicadores.error) message like so:
